...
export class CodeAssessComponent {
  ...
  async uploadFile() {
    this.messageService.add({
      severity: "info",
      summary: "Success",
      detail: "Uploading app...",
    });
    this.fetching = true;
    let formData = new FormData();
    formData.append("photo", this.uploadedFile![0]);
    fetch(getBaseURL("/api/rate_app"), { method: "POST", body: formData })
      .then((response) => {
        this.fetching = false;
        this.messageService.add({
          severity: "info",
          summary: "Success",
          detail: "Code assessment performed correctly",
        });
      ...
      .catch((error) => {
        ...
        this.messageService.add({
          severity: "info",
          summary: "Error",
          detail: "Error during code assessment",
        });
      });
  }
}
