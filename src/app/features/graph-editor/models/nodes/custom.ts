...
import { getColorFromCategory, getSocket } from "@shared/utils";
  ...
  implements Classic.Node
{
  ...
  constructor(nodeName: string, config: any) {
    ...
    for (let i = 0; i < config.inputs.length; i++) {
      ...
      this.addInput(
        input.port_label,
        new Classic.Input(getSocket(input.port_type), input.port_label)
      );
    }
    ...
    for (let i = 0; i < config.outputs.length; i++) {
      ...
      this.addOutput(
        output.port_label,
        new Classic.Output(getSocket(output.port_type), output.port_label)
      );
    }
  ...
  setData(data: any) {
    ...
    for (let key in this.params) {
      if (key in data.params) {
        ...
        if (data.params[key].show)
          this.params[key].show = data.params[key].show;
        if (data.params[key].isParam)
          this.params[key].isParam = data.params[key].isParam;
        if (data.params[key].param_label)
          this.params[key].param_label = data.params[key].param_label;
        if (data.params[key].optionId)
          this.params[key].optionId = data.params[key].optionId;
        if (data.params[key].type)
          this.params[key].type = data.params[key].type;
      }
    }
  }
